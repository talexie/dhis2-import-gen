 ...
 * @param {*} keys 
 */
export const createMoreColumns=(columns,keys)=>{
    if(!isEmpty(keys)){
        keys.forEach((k,i)=>{
            if(isPlainObject(k)){
                columns.push({
 ...
 * @param {*} name 
 */
export const exportDataToExcel=(data,cols,options={ bookType:'xlsx', bookSST:false, type:'binary' },filename="Excel Report",name="Sheet1")=>{
   ...
   // XLSX.utils.sheet_add_json(ws,data);
    //XLSX.utils.sheet_add_json(ws,dataRow?.row1,{ skipHeader:true,origin: "A1" });
    //XLSX.utils.sheet_add_json(ws,dataRow?.row2, { skipHeader: true, origin: "A2"});

    XLSX.utils.book_append_sheet(wb, ws, name);  
    XLSX.writeFile(wb, `${filename}.xlsx`, options);
